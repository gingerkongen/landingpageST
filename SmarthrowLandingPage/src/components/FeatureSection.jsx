import React from 'react'
import { features } from "../constants/Features.jsx";

const FeatureSection = () => {
  return (
    <div className="relative mt-20 border-b border-neutral-800 ">
      <div className="text-center">
        <span className="bg-neutral-100 text-orange-500 rounded-full h-6
        text-sm font-medium px-2 py-1 uppercase">
            FEATURES
        </span>
        <h2 className="text-3xl sm:text-5xl lg:text-6xl mt-10 lg:mt-20 tracking
        wide">
            Analyze training and match {" "} 
            <span className="bg-gradient-to-r from-orange-500 to-orange-800
            text-transparent bg-clip-text">
                data
            </span>
        </h2>
      </div>
     <div className="flex flex-wrap mt-10 mb-40 lg:mt-20">
        {features.map((f) => (
            <div key={f.id} className="w-full sm:w-1/2 lg:w-1/3">
            <div className="flex">
                <div className="flex mx-6 h-10 w-10 p-2 bg-neutral-100 text-orange-700 justify-center items-center rounded-full">
                <f.icon className="w-5 h-5" aria-hidden="true" />
                </div>
                <div>
                <h5 className="mt-1 mb-2 text-xl">{f.text}</h5>
                <p className="text-base text-neutral-600">{f.description}</p>
                </div>
            </div>
            </div>
        ))}
      </div>
    </div>
  )
}

export default FeatureSection
